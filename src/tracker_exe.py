...
# 캐싱이 필요하다면 전역 변수로 선언하여 한 번만 생성되게 합니다.
_supabase_instance = None

def get_supabase_client() -> Client | None:
    """
    데스크톱 배포용(.exe) Supabase 클라이언트 초기화
    환경 변수 대신 공용(Anon) 키를 직접 사용합니다.
    """
    global _supabase_instance
    
    # 이미 생성된 인스턴스가 있다면 재사용 (싱글톤 패턴)
    if _supabase_instance is not None:
        return _supabase_instance

    # ---------------------------------------------------------
    # [보안 안내] RLS 정책에서 INSERT만 허용했으므로 하드코딩해도 안전합니다.
    # ---------------------------------------------------------
    ...
    try:
        ...
        # 트래커 연결 실패가 메인 프로그램 실행을 방해하지 않도록 예외 처리
